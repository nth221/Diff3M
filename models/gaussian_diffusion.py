...
import numpy as np
import torch as th
from utils.train_util import visualize
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        assert t.shape == (B,)
        model_noise, pred_x_t = model(x, self._scale_timesteps(t), **model_kwargs)
        
        # print('---', model_noise.shape, pred_x_t.shape)
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            exit('wrong options for this project')
        else:
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            pred_xstart = process_xstart(
                    self._predict_xstart_from_eps(x_t=pred_x_t, t=t, eps=model_noise)
                )

            model_mean, _, _ = self.q_posterior_mean_variance(
                x_start=pred_xstart, x_t=pred_x_t, t=t
            )
    ...
    def training_losses(self, model,  x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_noise, pred_x_t = model(x_t, self._scale_timesteps(t), **model_kwargs)

            for idx, p_x_t in enumerate(pred_x_t):
                if self._scale_timesteps(t)[idx] <= 50:
                    generated_to_pil = ToPILImage()
                    p_x_t_img = generated_to_pil(p_x_t)

                    p_x_t_img.save('./results/test/' + str(self._scale_timesteps(t)[idx]) + '.jpg')

            if self.model_var_type in [
                ModelVarType.LEARNED,
                ModelVarType.LEARNED_RANGE,
            ]:
                print(self.model_var_type)
                exit('this is wrong option for this project.')

            noise_mse = mean_flat((noise - model_noise) ** 2) * 0.5
            pred_x_t_mse = mean_flat((x_t - pred_x_t) ** 2) * 0.5

            terms["mse"] = noise_mse + pred_x_t_mse
